package org.mj.study.zookeeper.simple;
...
public class JavaApiSample implements Watcher{
	...
	private static final String ZK_PATH = "/mingjing" ;
	 ...
	 */
	public void createZkConnection(String connectString,int sessionTimeout){
	...
	public boolean writeData(String path,String data){
		try {
			System.out.println("修改节点数据成功,path:"+path+",stat:"+this.zk.setData(path, data.getBytes(),-1));
		} catch (KeeperException e) {
			System.out.println( "修改节点数据失败，发生KeeperException" );
			e.printStackTrace();
		} catch (InterruptedException e) {
			System.out.println( "修改节点数据失败，发生InterruptedException" );
			e.printStackTrace();
		}
		return true;
	...
	public void deleteNode(String path){
		...
		try {
			...
			System.out.println( "删除节点数据成功，path:" +path);
		} catch (InterruptedException e) {
			System.out.println( "删除节点数据失败，发生InterruptedException" );
			e.printStackTrace();
		} catch (KeeperException e) {
			System.out.println( "删除节点数据失败，发生KeeperException" );
			e.printStackTrace();
		}
	}
	/** 
	...
	public static void main( String[] args ) { 
        ...
        sample.createZkConnection( CONNECTION_STRING, SESSION_TIMEOUT ); 
